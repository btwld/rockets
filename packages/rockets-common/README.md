...
> Rockets import path for the upstream Concepta motors (`repository`, `crud`, `hook`, `common`, `authentication`, `swagger-ui`) plus a handful of local helpers.
...
Usually pulled in transitively by `@bitwild/rockets` or `@bitwild/rockets-core`. Add explicitly when you import symbols from this package path.
...
This package never imports `@bitwild/rockets-core`. Core depends on common, not the other way around.
