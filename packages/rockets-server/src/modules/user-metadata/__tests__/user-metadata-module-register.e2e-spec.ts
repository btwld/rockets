/**
 * E2E bootstrap for {@link UserMetadataModule.register} and {@link UserModule.register}.
 * {@link RocketsModule} wires metadata via {@link rockets.module-definition} instead,
 * so default Rockets e2e never executes `user-metadata.module.ts`; this spec does.
 */
import { DynamicModule, Module, INestApplication, ValidationPipe } from '@nestjs/common';
...
describe('UserMetadataModule.register via UserModule (e2e)', () => {
  ...
  it('GET /me uses metadata service wired through UserMetadataModule.register()', async () => {
  ...
  it('PATCH /me exercises createOrUpdate through the same module graph', async () => {
    const repo = new UserMetadataRepositoryFixture();
    const moduleRef = await Test.createTestingModule({
      imports: [
        UserMetadataModuleRegisterE2eHarnessModule.forTest(baseOptions, repo),
