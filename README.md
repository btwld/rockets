...
**Status:** pre-1.0 (`1.0.0-alpha.9`, on npm under `@bitwild/*` with dist-tag
...
modules). `@bitwild/rockets-*` packages are mostly **curated re-exports plus
wiring**: `@bitwild/rockets-core` runs `buildAppRegistrationPlan` and turns your
...
**Path A — External auth** (`@bitwild/rockets`). You bring an
...
**Path B — Built-in auth** (`@bitwild/rockets-auth`). The framework owns the
...
**`@bitwild/rockets`** with **one shared identity** across the product.
...
Install the same packages as above plus `@bitwild/rockets-auth` and the upstream
...
- `defineFirebaseAuth({ forRoot | forRootAsync })` — Firebase Admin +
  `FirebaseAuthAdapter` (`@bitwild/rockets-adapter-firebase`).
- `defineRocketsAuth(...)` — built-in signup/login stack
  (`@bitwild/rockets-auth`); pair with `buildRocketsAuthResources()` on
  `resources`.
...
**Path B uses both** `@bitwild/rockets` and `@bitwild/rockets-auth`:
...
`@bitwild/rockets-core` for simple entity lifecycle rules, upstream
